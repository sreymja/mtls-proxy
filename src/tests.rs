...
async fn test_config_validation() {
    ...
    assert!(config.validate().is_err(), "Max request size 0 should be invalid");
    config.server.max_request_size_mb = 10; // Reset
    
    // Test invalid concurrent requests
    config.server.max_concurrent_requests = 0;
    assert!(config.validate().is_err(), "Max concurrent requests 0 should be invalid");
}
...
async fn test_tls_client_creation() {
    ...
    if cert_path.exists() && key_path.exists() {
        ...
        assert!(tls_client.is_ok(), "TLS client should be created successfully");
    } else {
...
fn test_hop_by_hop_header_filtering() {
    ...
    for header in hop_by_hop_headers {
        assert!(super::proxy::is_hop_by_hop_header(header), 
                "{} should be identified as hop-by-hop header", header);
    }
    ...
    for header in regular_headers {
        assert!(!super::proxy::is_hop_by_hop_header(header),
                "{} should not be identified as hop-by-hop header", header);
    }
}
...
async fn test_proxy_server_creation() {
    ...
    match proxy {
        ...
        Err(e) => println!("Proxy server creation failed (expected if no certificates): {}", e),
    }
}
