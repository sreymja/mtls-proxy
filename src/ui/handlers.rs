...
pub async fn logs_handler(
    ...
    let logs = log_manager.search_logs(
        None, // start_time
        None, // end_time
        method.as_deref(),
        status_code,
    ...
    let logs = logs.into_iter().skip(offset).take(limit).collect::<Vec<_>>();
...
async fn get_dashboard_stats(log_manager: Arc<LogManager>) -> serde_json::Value {
    ...
    let recent_logs = log_manager.search_logs(
        Some(one_hour_ago),
        Some(now),
        None,
        None,
    ...
    let daily_logs = log_manager.search_logs(
        Some(one_day_ago),
        Some(now),
        None,
        None,
